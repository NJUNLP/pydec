...
class TransformerEncoderBase(FairseqEncoder):
    ...
    def forward_scriptable(
        ...
        if decompose:
            c_x = pydec.zeros(x.size(), c_num=x.size(0)).to(x)
            x = pydec.diagonal_init(c_x, src=x, dim=0)
            # x.components[:] = 0
            # x = x.residual
...
class TransformerEncoderLayerBase(nn.Module):
    ...
    *cfg.encoder.normalize_before* to ``True``.
    ...
    def __init__(self, cfg):
        ...
        if cfg.activation_fn == "gelu":
            self.activation_fn = nn.functional.gelu
            # TODO: add float() to pydec
            # utils.get_activation_fn(activation=cfg.activation_fn)
        else:
