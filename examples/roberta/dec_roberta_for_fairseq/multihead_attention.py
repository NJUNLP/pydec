...
class MultiheadAttention(nn.Module):
    ...
    def forward(
        self,
        query,
        key: Optional[Union[Tensor, pydec.Composition]],
        value: Optional[Union[Tensor, pydec.Composition]],
        key_padding_mask: Optional[Tensor] = None,
    ...
    ) -> Tuple[Tensor, Optional[Tensor], Optional[pydec.Composition]]:
        ...
        # pydec
        def c2t(input: Union[pydec.Composition, Tensor]):
        ...
        if self.self_attention:
            q = self.q_proj(c2t(query))
            k = self.k_proj(c2t(query))
            v = self.v_proj(query)
        elif self.encoder_decoder_attention:
        ...
        # pydec: do not support add_bias_kv
        ...
        if saved_state is not None:
            ...
            if "prev_key" in saved_state:
                ...
                if static_kv:
                    ...
                    if isinstance(k, pydec.Composition):
                        assert isinstance(prev_key, pydec.Composition)
                        padding_num = k.numc() - prev_key.numc()
                        c_padding = pydec.zeros(prev_key.size(), c_num=padding_num).to(
                            prev_key
                        )
                        prev_key = pydec.c_cat([prev_key, c_padding], sum_residual=True)
            ...
            if "prev_value" in saved_state:
                ...
                if static_kv:
                    ...
                    # if v is Composition, the component number of v does not match that of prev_value.
                    # Padding a zero component to fix this.
                    if isinstance(v, pydec.Composition):
                        assert isinstance(prev_value, pydec.Composition)
                        padding_num = v.numc() - prev_value.numc()
                        c_padding = pydec.zeros(
                            prev_value.size(), c_num=padding_num
                        ).to(prev_value)
                        prev_value = pydec.c_cat(
                            [prev_value, c_padding], sum_residual=True
                        )
        ...
        # pydec: do not support add_zero_attn
        ...
        if self.encoder_decoder_attention and bsz != kv_bsz:
            ...
            if (
                isinstance(q, pydec.Composition)
                and isinstance(k, pydec.Composition)
                and q.numc() != k.numc()
            ):
                padding_num = q.numc() - k.numc()
                c_padding = pydec.zeros(k.size(), c_num=padding_num).to(k)
                k = pydec.c_cat([k, c_padding], sum_residual=True)
        ...
        if before_softmax:
            return attn_weights, v, value_composition
        ...
        # pydec : do not support q_bsz != kv_bsz
        assert bsz == kv_bsz
        if self.encoder_decoder_attention and bsz != kv_bsz:
            attn = torch.einsum(
                "bxhts,bhsd->bxhtd",
    ...
    ) -> Dict[str, Optional[Union[Tensor, pydec.Composition]]]:
        result = self.get_incremental_state(incremental_state, "attn_state")
        if result is not None:
            return result
        else:
            empty_result: Dict[str, Optional[Union[Tensor, pydec.Composition]]] = {}
            return empty_result
